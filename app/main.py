...
# 2. ANTHROPIC AI INTEGRATION
def get_claude_advice(income, expenses, savings_rate, emergency_months, debt_burden, investment_ratio, score):
    """Fetches personalized financial advice from Claude API."""
    try:
        if "ANTHROPIC_API_KEY" not in st.secrets:
            return "⚠️ API Key missing. Please add ANTHROPIC_API_KEY to Streamlit Secrets."
        
        client = anthropic.Anthropic(api_key=st.secrets["ANTHROPIC_API_KEY"])
        
        prompt = f"""You are 'MoneyMitra', an expert Indian Financial Advisor. 
        Analyze these financial metrics for an Indian user (values in INR):
        - Monthly Income: ₹{income:,.2f}
        - Monthly Expenses: ₹{expenses:,.2f}
        - Savings Rate: {savings_rate:.1f}%
        - Emergency Fund: {emergency_months:.1f} months
        - Debt Burden (EMI/Income): {debt_burden:.1f}%
        - Investment Ratio: {investment_ratio:.1f}%
        - Overall Financial Health Score: {score}/100

        Provide 5 SPECIFIC, ACTIONABLE recommendations. 
        Use exact rupee amounts where possible. 
        Focus on: Emergency fund gaps, high-interest debt, tax saving (80C/NPS), and long-term wealth creation.
        Be encouraging but direct. Keep each point concise."""
...
def calculate_financial_metrics(income, expenses, savings, debt, investments, emergency_fund):
    ...
    # Health Scoring (Out of 100)
    score = 0
    # 1. Savings Rate (Max 25 pts)
    score += min(25, (savings_rate / 30) * 25)
    # 2. Emergency Fund (Max 25 pts) - ideal 6 months
    score += min(25, (emergency_months / 6) * 25)
    # 3. Debt Burden (Max 25 pts) - ideal < 30% (inverted)
    score += max(0, 25 - (debt_burden / 2))
    # 4. Investment Ratio (Max 25 pts) - ideal 20%
...
def main_app():
    ...
    if submit or 'metrics' in st.session_state:
        ...
        with tab2:
            ...
            st.markdown("*Powered by Anthropic Claude 3.5 Sonnet*")
            
            if st.button("✨ Generate Personalized AI Advice"):
                with st.spinner("Claude is analyzing your numbers..."):
                    advice = get_claude_advice(
                        income, expenses, metrics['savings_rate'], 
                        metrics['emergency_months'], metrics['debt_burden'], 
                        metrics['investment_ratio'], metrics['score']
                    )
